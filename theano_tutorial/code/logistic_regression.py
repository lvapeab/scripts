...
class LogisticRegression(object): 
    ...
    The logistic regression is fully described by:
    a weight matrix :math:`W`
...
#(symbolic) loss for a given minibatch:
...
###############
 ...
 y = T.ivector('y') #labels presented as 1D vectors of [int] labels
...
#The cost we minimize during training is the negative log-likelihood of the model in symbolic format
 ...
 #Compiling a Theano function that computes the mistakes made by the model on a minibatch
 ...
 while (epoch < n_epochs) and (not done_looping):
  ...
  # if we got the best validation score until now
             if this_validation_loss < best_validation_loss:
                 ...
                 test_loses = [test_model(i)
                               for i in xrange(n_test_batches)]
                 test_score = numpy.mean(test_loses)
                 print(
                     (
